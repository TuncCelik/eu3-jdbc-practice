...
public class hrApiWithJsonPath {
    ...
    public void test1(){
        ...
        //SECOND SOLUTION---- get all country names where their region id is equal to 2--- SECOND SOLUTION
        List<Map> countries = jsonPath.getList("items");
        List<String> countryNamesWithRegionId2 = new ArrayList<>();

        for (Map country : countries) {
            if (country.get("region_id") == (Integer) 2){
                countryNamesWithRegionId2.add((String) country.get("country_name"));
            }
        }
    }
